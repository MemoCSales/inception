...
2. **Add user to sudo list**
	Follow this too: https://github.com/hanshazairi/42-born2beroot?tab=readme-ov-file 
...
8. **Create $HOME/.ssh/config file**
   ...
   ```sh
   # On host machine
   ssh-copy-id -i ~/.ssh/id_rsa.pub -p 4242 inception@localhost
   ```

   Test SSH Connection
   ```sh
   ssh -p 4242 inception@localhost
   ```
...
9. **Connection to VM using VSCODE**
   ...
   Check you have the extension Remote-SSH install first
...
` scp -r /path/to/local/directory Debian:/path/to/remote/directory`
   ...
   `scp -P 4242 /path/to/local/file username@localhost:/path/to/remote/directory`

   For copying entire directories:
   `scp -r -P 4242 /path/to/local/directory username@localhost:/path/to/remote/directory`
10. Send files thorough rsync:
```sh
# Sync a single file
rsync -av -e "ssh -p 4242" /path/to/local/file username@localhost:/path/to/remote/directory

# Sync a directory
rsync -avz -e "ssh -p 4242" /path/to/local/directory/ username@localhost:/path/to/remote/directory

# One-time sync with deletion:
rsync -avz --delete -e "ssh -p 4242" ~/project/ username@localhost:/home/username/project/

# Monitor changes:
rsync -avz --progress --stats -e "ssh -p 4242" ~/project/ username@localhost:/home/username/project/

# Verify Sync Status:
# Compare directories after sync
rsync -avzn -e "ssh -p 4242" ~/project/ username@localhost:/home/username/project/
```
Common rsync flags:
...
Specify correct local and remote paths
Note the trailing slash on source directory to copy contents instead of the directory itself
...
2. **Install Docker in VM for Ubunto/Debian:**
...
```sh
   ...
   -L: Follow redirects
   URL: https://download.docker.com/linux/ubuntu/gpg (Docker's GPG public key)
   Pipe (|) operation:

   Takes output from curl command
   Feeds it to the next command
   sudo gpg command components:

   --dearmor: Converts the ASCII-armored GPG key to binary format
   -o: Specifies output file
   Output path: /usr/share/keyrings/docker-archive-keyring.gpg
   Purpose:

   Downloads Docker's official GPG key
   Converts it to binary format
   Saves it to system keyring directory
   This key is used to verify Docker package signatures during installation
   This is a security measure to ensure you're installing authentic Docker packages.
```
